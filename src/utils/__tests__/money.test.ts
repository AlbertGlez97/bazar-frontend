...
// Una sola regla para el efectivo y para los precios del formulario de producto.
// Ante un texto ambiguo o sin forma de monto no se adivina.
const VALID_AMOUNTS: Array<[string, number]> = [
  ['1,000', 100000],
  ['1,000.50', 100050],
  ['1000', 100000],
  ['1000.5', 100050],
...
const AMBIGUOUS_AMOUNTS: string[] = [
  'abc',
  '1e5',
  '-20',
  '-10.50',
  ...
  '1.2.3',
  '12.34.56',
  '10.005', // no existen fracciones de centavo
  '1,000.505',
  '.',
  ',',
  ',5',
...
describe('parseMoneyText', () => {
  it.each(VALID_AMOUNTS)('%j -> %i centavos', (text, expected) => {
    expect(parseMoneyText(text)).toBe(expected)
  })

  it.each(AMBIGUOUS_AMOUNTS)('%j es ambiguo o inválido -> null', (text) => {
    expect(parseMoneyText(text)).toBeNull()
  })

  it('null, undefined y no-strings no se interpretan: null', () => {
    expect(parseMoneyText(null as unknown as string)).toBeNull()
    expect(parseMoneyText(undefined as unknown as string)).toBeNull()
    expect(parseMoneyText(12 as unknown as string)).toBeNull()
  })

  it('un monto fuera de los enteros exactos de JS no se interpreta: null', () => {
    expect(parseMoneyText('9,999,999,999,999,999')).toBeNull()
  })
  ...
  it('es consistente con Intl.NumberFormat("es-MX"): lo que se formatea así se lee de vuelta exacto', () => {
    ...
    for (const minor of [0, 1, 99, 100, 12550, 100000, 100050, 123456789, 2147483647]) {
      expect(parseMoneyText(fmt.format(minor / 100))).toBe(minor)
    }
  })
})

describe('parseCashInput', () => {
  it('es la misma implementación que parseMoneyText (una sola regla)', () => {
    expect(parseCashInput).toBe(parseMoneyText)
  })
...
// Envoltura de parseMoneyText: misma convención, pero ante lo ambiguo devuelve 0.
describe('displayToMinor', () => {
  it.each(VALID_AMOUNTS)('convierte %j pesos a %i centavos', (display, expected) => {
  ...
  it.each([
    ['125.50', 12550],
    ['125.5', 12550],
    ['1,000', 100000], // coma = miles, igual que en el campo de efectivo
    ['1,000.50', 100050],
  ])('lee %j como %i centavos', (display, expected) => {
    expect(displayToMinor(display)).toBe(expected)
  })

  it('acepta un number como entrada', () => {
    expect(displayToMinor(125.5)).toBe(12550)
    expect(displayToMinor(1000)).toBe(100000)
  })

  it.each(AMBIGUOUS_AMOUNTS)('%j es ambiguo o inválido -> 0 (nunca adivina)', (text) => {
    expect(displayToMinor(text)).toBe(0)
  })
  ...
  it('no acepta fracciones de centavo: 10.005 -> 0, no 1001', () => {
    expect(displayToMinor('10.005')).toBe(0)
    expect(displayToMinor('10.004')).toBe(0)
  })
})
